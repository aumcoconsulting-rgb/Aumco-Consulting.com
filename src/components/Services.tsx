...
const Services = () => {
  ...
  return (
    <section id="services" className="py-16 md:py-20 px-[5%] bg-background">
          ...
          <div className="flex gap-2 mb-9 flex-wrap">
            {tabs.map((tab, i) => (
              <button
                key={tab.label}
                onClick={() => setActive(i)}
                className={`font-body text-sm font-semibold px-5 md:px-6 py-2.5 rounded-lg border-[1.5px] cursor-pointer transition-all min-h-[44px] ${
                  active === i
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {tabs[active].items.map((item, i) => (
            <ScrollReveal key={item.name} delay={i * 80}>
              <div className="bg-card rounded-lg p-6 border border-input hover:shadow-[0_8px_40px_rgba(13,27,46,0.10)] hover:-translate-y-[3px] transition-all group">
